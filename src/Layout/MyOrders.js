...
const MyOrders = () => {
    ...
    const {data: myBookings = [], isLoading, refetch} = useQuery({
        ...
        queryFn: async() => {
            const res = await fetch(`https://bike-re-sale-server.vercel.app/bookings?email=${user?.email}`,{
               headers: {
    ...
    if(isLoading){
      return (
        <div className='flex justify-center'>
         <button disabled type="button" className="text-white mt-32 bg-black focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 dark:bg-black dark:hover:bg-black inline-flex items-center">
    ...
    const handleDelete = (id) => {
        fetch(`https://bike-re-sale-server.vercel.app/bookings/${id}`, {
           method: 'DELETE'
        })
        .then(res => res.json())
        .then(data => {
    ...
    return (
        <div className="overflow-x-auto h-5/6">
         { myBookings.length ?
           <React.Fragment>
            <h1 className='text-4xl font-semibold mt-4 mb-6'>My Bookings</h1>  
            ...
            { myBookings ?
              myBookings?.map((myBook, i) => <tr key={myBook._id}>
                ...
                <td><button onClick={() => handleDelete(myBook._id)} className='btn btn-sm bg-black'>Remove</button></td>
               </tr> )
               :
